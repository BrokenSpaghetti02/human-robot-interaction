...
#------------------------------------------------- Chat with Gemini Pro with Intention Detection -------------------------------------------------
        ...
        combined_input = user_input + " (sentence not in bracket is user input. Please detect if the user has the intention to play a rock paper scissors game. If yes, reply with 'INTENT_PLAY_GAME'. Otherwise, reply with 'NO_INTENT'. Also, detect if the user has the intention to recognize a face. If yes, reply with 'INTENT_RECOGNIZE_FACE'.)"
        ...
        filtered_result = result.replace('INTENT_PLAY_GAME', '').replace('NO_INTENT', '').replace('INTENT_RECOGNIZE_FACE', '').strip()
